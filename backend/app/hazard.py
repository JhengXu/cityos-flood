...
- 校准教师当前用 `model.MODEL`（混合物理）作为占位教师；
...
CALIB_PROVENANCE = (
    "estimated (calibrated to hybrid teacher model.MODEL); "
...
def _teacher_risk_seq(did, R_seq):
    """占位教师：使用混合物理模型 model.MODEL。可替换为真实水位标定。"""
    ...
    for R in R_seq:
        cum = min(cum + R, 300.0)
        out.append(model.MODEL.predict_one(R, cum, C, V)["prob"])
