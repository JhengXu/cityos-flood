...
def _build_snapshot(latest, source, locations):
    ...
    for station in stations:
        try:
            observed = datetime.fromisoformat(str(station["time"]).replace("Z", "+00:00"))
    ...
    observed_at = max(parsed_times) if parsed_times else None
    age_hours = (
        max(0.0, (now - observed_at.astimezone(now.tzinfo)).total_seconds() / 3600.0)
        if observed_at
        else None
    )
    return {"source": source, "ok": True, "count": len(stations), "flooding_count": len(flooding),
            ...
            "freshness": "fresh" if age_hours is not None and age_hours <= 3.0 else "stale"}


def _schedule_live_refresh():
    """Start at most one credentialed refresh while serving the last snapshot."""
    global _REFRESHING
    if not _has_opendata():
        return False
    with _REFRESH_LOCK:
...
def fetch_waterlevel():
    ...
    if _WL_CACHE["data"]:
        refreshing = _schedule_live_refresh()
        snapshot = dict(_WL_CACHE["data"])
        snapshot["cache_status"] = "refreshing" if refreshing else "expired-no-credentials"
        return snapshot
    ...
    if _SZ_WL_CACHE:
        refreshing = _schedule_live_refresh()
        snapshot = _build_snapshot(_SZ_WL_CACHE, "cache(shenzhen-flood 真实数据)", locations)
        snapshot["cache_status"] = "refreshing" if refreshing else "historical-cache"
        return snapshot
