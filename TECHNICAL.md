...
| 排水防涝设计标准 | ⚠️ 代表性估算 | — | 应替换为市政排水管网台账 |
| 低洼比例 / 不透水率 / 临海度 | ⚠️ 代表性估算 | — | 应替换为下垫面 / GIS 分析结果 |
| 泵站能力台账 | ⚠️ 代表性估算 | — | 应替换为水务局真实泵站数据 |
| LSTM 模型权重 | ✅ 真实训练产物 | 本地合成数据训练并缓存 | `data/lstm_weights.npz`（详见 §4） |

> 关键结论：**天气/高程/历史事件是真实接入的**；城市排水类市政特征是估算值、留有替换缝。当前深圳实时降雨很小（约 0~0.6 mm/h），所以仪表盘实时风险多为「低」——这是真实情况，不是假数据。要看模型能力，请用 SIMULATE 情景沙盘。
