...
export default function OceanBoundaryPanel({ sim }) {
  ...
  const requestedOffset = ocean.requested_extra_rain_peak_offset_h
  ...
  return (
    <section className="card stage ocean-card">
      ...
      <div className="ocean-kpis">
        ...
        <div className="ov-card"><div className="ov-k">实际总雨峰−潮峰</div><div className="ov-v">{offset == null ? '—' : `${offset > 0 ? '+' : ''}${offset}h`}</div><div className="ov-sub">额外雨峰目标 {requestedOffset == null ? '—' : `${requestedOffset > 0 ? '+' : ''}${requestedOffset}h`}</div></div>
