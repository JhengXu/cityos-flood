...
import { getCurrent, uploadData, manualForecast, depthQuantilesM, formatDepthM, formatPercent } from '../api'
...
export default function DataLabPanel({ onDataUpdated }) {
  ...
  async function onFile(f) {
    ...
    try {
      ...
      if (res.status === 'ok' || res.status === 'accepted') {
        const readiness = res.data_readiness || res.readiness
        const developmentReady = readiness?.forecast_training_ready || readiness?.eligible_for_model_development
        const blockers = readiness?.blockers?.length ? ` 阻塞：${readiness.blockers.join('；')}` : ''
        setUpMsg(`已校验并入库 "${res.saved || f.name}"（${res.rows ?? '—'} 行）。${developmentReady ? '已达到候选开发门槛，仍需事件外验证。' : '未自动训练或发布指标；需继续累积独立积水事件。'}${blockers}`)
  ...
  const foreHasDepth = fore?.trajectory?.some((point) => depthQuantilesM(point).available)
  const fc = fore?.trajectory ? fore.trajectory.map((point) => {
    const depth = depthQuantilesM(point)
    return foreHasDepth ? {
      h: point.h || point.time,
      value: depth.p50,
      interval: [depth.p10, depth.p90],
    } : {
      h: point.h || point.time,
      value: (point.prob || 0) * 100,
      interval: [(point.lo || 0) * 100, (point.hi || 0) * 100],
    }
  }) : null
  ...
  return (
    <section className="card stage">
      <div className="card-h">
        数据实验室 · 最新数据 + 用户输入
        <span className="hint">下一小时降雨预报 / 观测契约校验入库 / 手动边界条件集合推演</span>
      </div>
      ...
      <div className="datalab-grid">
        <div className="dl-cell">
          ...
          <div className="dl-src">{current ? `数据源：${current.data_source} · 有效时刻 ${current.valid_at || current.hour || ''}` : '加载中…'}</div>
        ...
        {/* 上传真实数据 */}
        <div className="dl-cell">
          <div className="dl-h">上传观测数据 → 校验入库</div>
          ...
          <div className="drop-hint">建议列：timestamp,event_id,district_id,rainfall_mm,water_depth_m,available_at（时间须含时区）</div>
          {uploading && <div className="empty">校验与入库中…</div>}
          {upMsg && <div className="push-ok">{upMsg}</div>}
        </div>

        {/* 手动输入预测 */}
        <div className="dl-cell">
          <div className="dl-h">手动输入 → 集合水深推演</div>
          ...
          {fore && (
            <div className="fore-res">
              <div className="fore-peak">
                峰值 {fore.peak_depth_p50_m != null ? formatDepthM(fore.peak_depth_p50_m) : formatPercent(fore.peak_prob)} · 等级 <b style={{ color: '#e08a1e' }}>{fore.peak_level}</b>
              </div>
              <ResponsiveContainer width="100%" height={140}>
                <LineChart data={fc} margin={{ top: 5, right: 8, bottom: 0, left: -18 }}>
                  ...
                  <YAxis tick={{ fill: '#8C9098', fontSize: 10 }} domain={foreHasDepth ? ['auto', 'auto'] : [0, 100]} unit={foreHasDepth ? 'm' : '%'} />
                  <Tooltip contentStyle={{ background: '#0B0D10', border: '1px solid rgba(255,255,255,.12)', color: '#F3F3EF' }} />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
                  <Area dataKey="interval" stroke="none" fill="rgba(20,91,255,.18)" name={foreHasDepth ? 'P10–P90' : '兼容区间'} />
                  <Line type="monotone" dataKey="value" stroke="#145BFF" strokeWidth={2} dot={false} name={foreHasDepth ? 'P50 水深' : '兼容概率'} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
