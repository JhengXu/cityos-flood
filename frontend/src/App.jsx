...
import WamOptimizationPanel from './components/WamOptimizationPanel.jsx'
...
export default function App() {
  ...
  async function load() {
    ...
    try {
      ...
      if (requestId === loadRequestId.current) setError(e.message || '加载失败')
    } finally {
      if (requestId === loadRequestId.current) setLoading(false)
    }
  }

  async function runSim(params, forecastRunId = data?.forecast_run_id) {
    const requestId = ++simRequestId.current
    const expectedRunId = forecastRunId
    setSimLoading(true)
    try {
      const r = await getSimulate({
        ...params,
        forecast_days: data?.forecast_days || 3,
        ...(forecastRunId ? { forecast_run_id: forecastRunId } : {}),
      })
      if (
        requestId !== simRequestId.current
      ...
      if (requestId === simRequestId.current) setSimLoading(false)
    }
  }
  ...
  useEffect(() => {
    if (data?.forecast_run_id) runSim({ preset: 'typhoon_tide' }, data.forecast_run_id)
  }, [data?.forecast_run_id])

  const view = useMemo(() => {
    if (!data) return null
    return data.districts.map((d) => {
      const series = d.series || []
      return { ...d, at: series[Math.min(hour, series.length - 1)] || d.current || {} }
    })
  }, [data, hour])
  ...
  return (
    <div className="app">
      ...
      <nav className="topnav">
        ...
        <a href="#wam-optimize" className="tn-link">自主优化 WAM</a>
        <div className="tn-spacer" />
        <span className="tn-tag">守恒集合模型 · 深圳内涝</span>
      </nav>
        ...
        <>
          ...
          <section id="research" className="research sec-block">
            <div className="section-title">
              模型研究 · 数据就绪度 + 可审计推演 + 独立验证边界
              <span className="st-sub">城市空间特征 → 守恒状态空间集合 → 观测同化 → 事件外验证</span>
            </div>
            ...
            <ErrorBoundary fallback={<div className="card"><div className="err-box">真实数据态势加载失败</div></div>}>
              <RealDataPanel predictData={data} />
            </ErrorBoundary>
            <ErrorBoundary fallback={<div className="card"><div className="err-box">街道级风险加载失败</div></div>}>
              <StreetRiskPanel forecastDays={data.forecast_days} forecastRunId={data.forecast_run_id} />
            </ErrorBoundary>
          ...
          <section id="simulate" className="simulate sec-block">
            ...
            <ErrorBoundary fallback={<div className="card"><div className="err-box">自主优化 WAM 模块加载失败</div></div>}>
              <WamOptimizationPanel predictData={data} />
            </ErrorBoundary>
      ...
      <footer className="foot">
        CITY OS · 深圳城市内涝集合推演 ｜ 驱动：Open-Meteo 多点降雨预报 + DEM / WorldCover / OSM 派生城市特征 +
        CHIRPS 逐日降雨 + 短时水位质控缓存。行政区尺度结果仅用于研究推演；独立灾情事件真值不足时，不声称已验证预测准确率。
      </footer>
    </div>
  )
}
