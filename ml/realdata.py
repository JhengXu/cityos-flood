# -*- coding: utf-8 -*-
"""旧时序实验的观测输入接入（P0 · shenzhen_p0_data）。

这里可以读取真实降雨和水位观测，但在测站映射、事件级深度标签和可用时间审计
完备以前，生成的监督目标仍是规则派生的代理标签，不能称为真实监督，也不能用来
证明 LSTM/Transformer 的真实预报技巧。
  ...
  可用观测与标签限制：
    - 15_..._flood_water_level                   : 积涝点水位（CZBM 测站，SW 水位、SJ 时间）
      ...
      真正的分区水位标签。当前分区标签 = 降雨超额规则 ∩ 历史事件受影响区，属于代理标签。

该模块只为显式开启的历史复现实验保留；在线预测不导入它。
"""
import argparse
import os
import sys
...
class ProxyLabelOptInRequired(RuntimeError):
    """未显式允许代理标签时，拒绝构造旧监督学习目标。"""


def _require_proxy_label_opt_in(allow_proxy_labels):
    if not allow_proxy_labels:
        raise ProxyLabelOptInRequired(
            "realdata 只能构造规则派生代理标签。仅为复现历史实验时可显式传 "
            "allow_proxy_labels=True；产物不得用于真实预测能力声明。"
        )
...
def _water_level_affected(event_date):
    """若映射和事件期水位可用，返回超过未校准阈值的分区集合。

    这仍是代理判据，不是经过零点/阈值审计的独立积水深度标签。
...
# ============ 组装观测输入 + 历史代理标签样本 ============
def build_real_event_samples(allow_proxy_labels=False):
    """
    对每个有观测降雨的事件构建历史代理标签样本。
    标签由降雨超额规则与事件受影响区派生，不是独立积水深度真值。
    默认 fail closed；仅 ``allow_proxy_labels=True`` 可用于历史复现。
    返回 list of sample dict: {X:(T,5), y:(T,), meta:{event,district,peak,affected,real}}
    """
    _require_proxy_label_opt_in(allow_proxy_labels)
        ...
        for did, series in rain_by_dist.items():
            ...
            samples.append({
                ...
                "meta": {"event": date, "district": did, "peak": peak,
                         ...
                         "observational_inputs": True, "proxy_label": True,
                         "independent_label": False},
            })
    return samples


def build_real_event_series(allow_proxy_labels=False):
    """
    供 replay（提前量）使用：返回 dict (event,district) ->
    {hours, rain, affected, drainage, vuln}。用真实降雨构建完整序列，便于滚动预测。
    replay 会由该序列派生代理目标，因此同样要求显式 opt-in。
    """
    _require_proxy_label_opt_in(allow_proxy_labels)
    if not REAL_DATA_ENABLED:
        return {}
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="复现观测输入 + 代理标签的旧实验")
    parser.add_argument(
        "--allow-proxy-labels",
        action="store_true",
        help="显式允许构造规则派生代理标签（不得用于真实技巧声明）",
    )
    args = parser.parse_args(argv)
    if not args.allow_proxy_labels:
        parser.error("必须显式提供 --allow-proxy-labels；代理标签默认禁用")

    print("P0_ROOT:", P0_ROOT, "| enabled:", REAL_DATA_ENABLED)
    print("格点数:", len(_GRID_LATLON) if _GRID_LATLON else "(未加载)")
    s = build_real_event_samples(allow_proxy_labels=True)
    print("观测输入 + 代理标签样本数:", len(s))
    for s0 in s[:3]:
