...
def replay(model, allow_proxy_labels=False):
    """历史代理标签回放；默认拒绝运行。

    即使输入降雨是观测值，比较目标仍由超排水规则派生，不能称为真实提前量。
    """
    if not allow_proxy_labels:
        raise ds.InsufficientIndependentLabels(
            "历史回放目标是规则派生代理标签；请仅在复现实验时显式允许。"
        )
    import torch
    try:
        ...
        real_series = (
            ...
            if realdata.REAL_DATA_ENABLED else {}
        )
    except Exception:
        real_series = {}
    if real_series:
        ...
        for (event, did), info in real_series.items():
            ...
            rows.append({
                ...
                "lead_h": lead,
                "observational_inputs": True,
                "proxy_target": True,
            })
        return rows
    # 回退：原有单窗口回放（合成/演示数据）
    samples = ds.build_samples(allow_proxy_labels=True)
    H = config.HORIZON
    rows = []
    for s in samples:
        X = s["X"]
...
def run(allow_proxy_labels=False):
    d = ds.load(allow_proxy_labels=allow_proxy_labels)
    import torch
    from .train import load_model

    model = load_model()
    rep = aggregate(d["test"][0], d["test"][1], d["test_meta"], model)
    rep["replay"] = replay(model, allow_proxy_labels=allow_proxy_labels)
    rep.update({
        "invalid_for_skill_claim": True,
        "valid_for_skill_claim": False,
        "independent_event_labels": False,
        "label_provenance": (
            "proxy labels derived from rainfall/drainage rules and affected-area facts"
        ),
        "warning": (
            "Historical reproduction only: these metrics do not measure real-world flood "
            "forecast skill."
        ),
    })
...
def fmt_report(rep):
    lines = ["【警告】代理标签历史实验；以下指标不能用于真实预测能力声明。"]
    lines.append("【测试集总体】")
